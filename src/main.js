...
function playSound(name, delay = 0) {
  ...
  const audio = soundBank.get(name);
  if (!audio) return;
  const playFallback = () => {
    const volume = SOUND_VOLUMES[name] ?? 1;
    const copies = Math.max(1, Math.ceil(volume));
    for (let index = 0; index < copies; index += 1) {
      const player = index === 0 ? audio : audio.cloneNode(true);
      player.pause();
      player.volume = Math.min(volume, 1);
      try {
        player.currentTime = 0;
      } catch {
        // Some browsers cannot seek before metadata is available. Playing still works.
      }
  ...
  if (delay > 0) {
    setTimeout(playFallback, delay * 1000);
    return;
  }
  playFallback();
}
...
function prepareSounds() {
  ...
  Object.entries(SOUNDS).forEach(([name, src]) => {
    ...
    audio.preload = "auto";
    audio.volume = Math.min(SOUND_VOLUMES[name] ?? 1, 1);
    audio.load();
    soundBank.set(name, audio);
    loadSoundBuffer(name, src);
  });
}
