...
const SOUND_VERSION = "20260827-ambient-bg-2";
const SOUNDS = {
...
const BACKGROUND_SOUND_URL = "./assets/sounds/eerie-ambient.mp3";
...
let backgroundPlayPromise = null;
...
window.addEventListener("pointerdown", startBackgroundSound, { passive: true });
...
function soundUrl(src) {
  const separator = src.includes("?") ? "&" : "?";
  return `${src}${separator}v=${SOUND_VERSION}`;
}
...
function playSound(name, delay = 0) {
  ...
  const start = () => {
    const player = new Audio(soundUrl(SOUNDS[name]));
    player.loop = false;
    player.autoplay = false;
    player.preload = "auto";
    player.volume = Math.min(SOUND_VOLUMES[name] ?? 1, 1);
    activeEffectPlayers.add(player);
    const cleanup = () => activeEffectPlayers.delete(player);
...
function prepareBackgroundSound() {
  ...
  backgroundAudio = new Audio(soundUrl(BACKGROUND_SOUND_URL));
  ...
  requestBackgroundPlayback(true);
}

function startBackgroundSound() {
  if (!SOUND_ENABLED || !backgroundAudio) return;
  requestBackgroundPlayback(false);
}

function requestBackgroundPlayback(muted) {
  backgroundAudio.loop = true;
  backgroundAudio.muted = muted;
  backgroundAudio.volume = BACKGROUND_VOLUME;
  activeEffectPlayers.forEach((player) => (player.loop = false));
  if (!backgroundAudio.paused || backgroundPlayPromise) return;
  backgroundPlayPromise = backgroundAudio
    .play()
    .catch(() => {})
    .finally(() => {
      backgroundPlayPromise = null;
    });
}
...
function collectParts() {
  ...
  gltfRoot.traverse((object) => {
    ...
    if (name.startsWith("Glass_Cracked_") || name === "Clock_Case_Cracked")
      parts.cracks.push(object);
    ...
    if (name === "Clock_Case_Normal" || name === "Clock_Frame_Normal")
      parts.caseNormal.push(object);
    if (
      [
        "Clock_Dial_Normal",
        "Clock_Dial_Inner_Ring",
        "Clock_Hand_Hub",
        "Clock_Dial_Decor_Left",
        "Clock_Dial_Decor_Right",
      ].includes(name)
    ) {
...
function configureInitialVisibility() {
  ...
  [...parts.glassNormal, ...parts.caseNormal, ...parts.looseParts].forEach(
...
function createMysticRebuildEffect() {
  ...
  const ring = new THREE.Mesh(
    new THREE.TorusGeometry(1.22, 0.014, 14, 128),
    ringMaterial,
  );
  mysticGroup.add(ring);

  const tiltedRing = new THREE.Mesh(
    new THREE.TorusGeometry(1.42, 0.008, 12, 128),
    ringMaterial.clone(),
  );
...
function onPointerMove(event) {
  ...
  if (dragState.active) {
    ...
    if (dragState.rotateClock && dragState.moved) {
      ...
      userRotation.x = THREE.MathUtils.clamp(
        userRotation.x + dy * 0.006,
        -0.8,
        0.8,
      );
    }
  ...
  const hit = raycaster
    .intersectObjects(parts.clickTargets, true)
    .some((entry) => entry.object.visible);
...
function onPointerUp(event) {
  ...
  endDrag(event);
  if (wasClick && state.ready && state.introDone && !state.stageAnimating)
    advanceDamageStage();
}
...
function endDrag(event) {
  ...
  canvas.style.cursor = state.hoverClock
    ? "grab"
    : state.damageStage > 0
      ? "pointer"
      : "default";
...
function tweenPlanProgress(groupName, targetProgress, duration, easing) {
  ...
  return tween(duration, (progress) => {
    ...
    tweenStarts.forEach((start) => {
      ...
      start.plan.object.position.lerpVectors(
        start.position,
        motion.position,
        eased,
      );
...
function getPlanMotionAtProgress(plan, progress) {
  ...
  const wobble =
    Math.sin(arcProgress * Math.PI * 4.2 + plan.spinPhase) *
    plan.spinWobble *
    flightArc;
  const position = new THREE.Vector3().lerpVectors(
    plan.originalPosition,
    plan.toPosition,
    progress,
  );
  ...
  rotation.x =
    THREE.MathUtils.lerp(plan.originalRotation.x, plan.toRotation.x, progress) +
    wobble * 0.42;
  rotation.y =
    THREE.MathUtils.lerp(plan.originalRotation.y, plan.toRotation.y, progress) -
    wobble * 0.35;
  rotation.z =
    THREE.MathUtils.lerp(plan.originalRotation.z, plan.toRotation.z, progress) +
    wobble * 0.28;
  return { position, rotation };
}
...
function applyTweenedRotation(plan, startRotation, targetRotation, progress) {
  if (isClockHand(plan.object) || plan.object === parts.pendulum) {
    plan.object.rotation.x = THREE.MathUtils.lerp(
      startRotation.x,
      targetRotation.x,
      progress,
    );
    plan.object.rotation.y = THREE.MathUtils.lerp(
      startRotation.y,
      targetRotation.y,
      progress,
    );
  ...
  plan.object.rotation.z = THREE.MathUtils.lerp(
    startRotation.z,
    targetRotation.z,
    progress,
  );
...
function createExplosionPlans(objects, options = {}) {
  ...
  return objects.map((object, index) => {
    ...
    if (direction.length() < 0.001) {
      direction.set(
        Math.cos(index * 2.17),
        Math.sin(index * 1.31) * 0.4,
        Math.sin(index * 2.17),
      );
    }
    direction.normalize();

    const chaoticDirection = direction
      .clone()
      .add(
        new THREE.Vector3(
    ...
    const blastDistance =
      baseDistance + pseudoNoise(index + 39, seed) * distanceJitter;
    const screenKick = new THREE.Vector3(
      sideSign * sideBlast * (0.45 + pseudoNoise(index + 47, seed) * 0.95),
      verticalSign *
        verticalBlast *
        (0.35 + pseudoNoise(index + 53, seed) * 1.1),
      0,
    );
    const cameraKick = cameraDirection
      .clone()
      .multiplyScalar(
        cameraSign * cameraBlast * (0.35 + pseudoNoise(index + 61, seed) * 0.9),
      );

    const targetWorld = worldPosition
      .clone()
    ...
    const curveOffset = worldVectorToParentLocal(
      object,
      worldPosition,
      curveWorld,
    );
    const spin = new THREE.Euler(
      ...
      original.rotation.y +
        (pseudoNoise(index + 13, seed) - 0.5) * spinStrength * 1.2,
      original.rotation.z +
        (pseudoNoise(index + 29, seed) - 0.5) * spinStrength,
    );
...
function worldVectorToParentLocal(object, worldOrigin, worldVector) {
  ...
  const localTarget = object.parent.worldToLocal(
    worldOrigin.clone().add(worldVector),
  );
...
async function vortexBeforeRebuild(plans) {
  ...
  const vortexCenterWorld = clockRoot
    .getWorldPosition(new THREE.Vector3())
    .add(new THREE.Vector3(0, 0.1, 0));
  const starts = plans.map((plan, index) => {
    const worldPosition = plan.object.parent.localToWorld(
      plan.object.position.clone(),
    );
  ...
  await tween(1.85, (progress) => {
    ...
    starts.forEach((start) => {
      ...
      const targetWorld = new THREE.Vector3(
        ...
        vortexCenterWorld.y +
          Math.sin(angle) * start.radius * radiusPulse +
          glow * 0.24,
        vortexCenterWorld.z +
          start.offset.z * (1 - eased * 0.1) +
          Math.sin(angle * 1.4 + start.phase) * glow * 0.42,
      );

      start.plan.object.position.copy(
        start.plan.object.parent.worldToLocal(targetWorld),
      );
      start.plan.object.rotation.x =
        start.rotation.x +
        spin * 0.18 +
        Math.sin(start.phase + progress * Math.PI * 4) * glow * 0.22;
      start.plan.object.rotation.y =
        start.rotation.y -
        spin * 0.12 +
        Math.cos(start.phase + progress * Math.PI * 3) * glow * 0.2;
      if (
        !isClockHand(start.plan.object) &&
        start.plan.object !== parts.pendulum
      ) {
...
async function rebuildClock(plans) {
  ...
  await tween(2.45, (progress) => {
    ...
    rebuildStarts.forEach((start) => {
      start.plan.object.position.lerpVectors(
        start.position,
        start.plan.originalPosition,
        eased,
      );
      start.plan.object.rotation.x = THREE.MathUtils.lerp(
        start.rotation.x,
        start.plan.originalRotation.x,
        eased,
      );
      start.plan.object.rotation.y = THREE.MathUtils.lerp(
        start.rotation.y,
        start.plan.originalRotation.y,
        eased,
      );
      if (
        !isClockHand(start.plan.object) &&
        ...
        start.plan.object.rotation.z = THREE.MathUtils.lerp(
          start.rotation.z,
          start.plan.originalRotation.z,
          eased,
        );
      }
    });
    ...
    parts.cracks.forEach((object) =>
      setMeshOpacity(object, (1 - eased) * 0.82),
    );
  });
...
function resetSceneToWholeClock() {
  ...
  [...damagePlans.glass, ...damagePlans.case, ...damagePlans.loose].forEach(
    resetAirMotionRecord,
  );
}
...
function updateCameraShake(delta) {
  ...
  const progress = THREE.MathUtils.clamp(
    shakeState.elapsed / shakeState.duration,
    0,
    1,
  );
  const heartbeat = Math.sin(progress * Math.PI * 2.4);
  const envelope =
    Math.pow(1 - progress, 2.15) * (0.72 + Math.abs(heartbeat) * 0.28);
...
function updateAirMotion(plan, strength) {
  ...
  const desiredOffset = plan.floatVector
    .clone()
  ...
  const desiredRotation = plan.airSpin
    .clone()
    .multiplyScalar(strength * spinPulse);
...
function isClockHand(object) {
  return (
    object === parts.hourHand ||
    object === parts.minuteHand ||
    object === parts.secondHand
  );
}
...
function updateClockLife(delta) {
  ...
  clockRoot.rotation.x =
    userRotation.x + Math.sin(state.elapsed * 0.48) * 0.012;
  clockRoot.rotation.y = userRotation.y + state.vortexYaw;
  clockRoot.rotation.z = drift + state.vortexSpin;

  if (parts.hourHand)
    parts.hourHand.rotation.z = handStart.hour - state.elapsed * 0.045;
  if (parts.minuteHand)
    parts.minuteHand.rotation.z = handStart.minute - state.elapsed * 0.26;
  if (parts.secondHand)
    parts.secondHand.rotation.z = handStart.second - state.elapsed * 2.25;
  if (parts.pendulum)
    parts.pendulum.rotation.z =
      pendulumStartZ + Math.sin(state.elapsed * 1.55) * 0.22;
